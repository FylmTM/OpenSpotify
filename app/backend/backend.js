  ...
  static bindEvents() {
    ...
    ipcMain.on('spotify-web-auth', (event) => {
      ...
      spotifyAuth.authorize()
        .then(data => event.sender.send('spotify-web-auth-result', { type: 'success', data }))
        .catch(error => event.sender.send('spotify-web-auth-result', { type: 'error', error: error.toString() }));
    });
